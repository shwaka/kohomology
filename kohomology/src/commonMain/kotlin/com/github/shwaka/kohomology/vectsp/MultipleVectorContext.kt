...
public open class MultipleVectorContext<S : Scalar, V : NumVector<S>>(
...
) : NumVectorContext<S, V> by NumVectorContext(numVectorSpace) {
    ...
    @Suppress("UNCHECKED_CAST")
    public operator fun <B : BasisName> Vector<B, S, V>.minus(other: Vector<B, S, V>): Vector<B, S, V> {
        if (this.vectorSpace != other.vectorSpace)
    ...
    public operator fun <B : BasisName> S.times(vector: Vector<B, S, V>): Vector<B, S, V> = vector * this
    public operator fun <B : BasisName> Vector<B, S, V>.times(scalar: Int): Vector<B, S, V> = this * scalar.toScalar()
    public operator fun <B : BasisName> Int.times(vector: Vector<B, S, V>): Vector<B, S, V> = vector * this.toScalar()
    public operator fun <B : BasisName> Vector<B, S, V>.unaryMinus(): Vector<B, S, V> = this * (-1)
}
