...
private fun <S : Scalar, V : NumVector<S>, M : Matrix<S, V>> testWithCyclicGroupQuot(
    ...
    val chainMap = ChainMapAlongAlgebraMapFromFreeComplex(
        source = sourceResol,
        target = targetResol,
        ...
        ModuleMapAlongAlgebraMapFromFreeModule.fromValuesOnGeneratingBasis(
            source = sourceModule,
            target = targetModule,
            algebraMap = algebraMap,
            values = values,
...
class ChainMapAlongAlgebraMapFromFreeComplexTest : FreeSpec({
    tags(chainMapTag)
