...
private fun <S : Scalar, V : NumVector<S>, M : Matrix<S, V>> testWithScalarMultiplication(
    ...
    "test multiplication by $scalar on the free resolution over $field[Z/$order]" - {
        ...
        "it should induce zero or isomorphism depending on the characteristic" {
            (-maxDeg..maxDeg).forAll { n ->
                val degree = IntDegree(n)
                if (scalar % p == 0) {
                    chainMap.underlyingDGLinearMap[degree].isZero().shouldBeTrue()
                } else {
                    chainMap.underlyingDGLinearMap[degree].isIsomorphism().shouldBeTrue()
                }
            }
        }
...
class FreeChainMapTest : FreeSpec({
    tags(chainMapTag)
