...
tools_json=$(jq '.tools' < "$json_file")
targets_json=$(jq '.targets' < "$json_file")
...
for tool in $(echo "$tools_json" | jq --raw-output '.[]'); do
    ...
    result_tool_bench="{}"
    for target in $(echo "$targets_json" | jq --raw-output 'keys | .[]'); do
        result_target="[]"
        degrees=$(echo "$targets_json" | jq --arg target "$target" '.[$target].degrees | .[]')
...
export const tools = $(echo "$tools_json") as const
export type Tool = (typeof tools)[number]

export const targets = $(echo "$targets_json" | jq keys) as const
